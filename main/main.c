...
void save_score(int game_time, int game_mode, int score, int total) {
  ...
  snprintf(key_last_score, sizeof(key_last_score), "s%d%d_ls", game_time,
           game_mode);
  snprintf(key_last_total, sizeof(key_last_total), "s%d%d_lt", game_time,
           game_mode);
  snprintf(key_best_score, sizeof(key_best_score), "s%d%d_bs", game_time,
           game_mode);
  snprintf(key_best_total, sizeof(key_best_total), "s%d%d_bt", game_time,
           game_mode);

  ESP_LOGI("SCORE", "Saving scores for game_time=%d, game_mode=%d", game_time,
           game_mode);
  ESP_LOGI("SCORE",
           "Keys: last_score='%s', last_total='%s', best_score='%s', "
           "best_total='%s'",
  ...
  if (err != ESP_OK)
    ESP_LOGE("SCORE", "Failed to set last_score: err=0x%x", err);
  else
    ESP_LOGI("SCORE", "Last score saved: %d", score);

  err = nvs_set_i32(nvs, key_last_total, total);
  if (err != ESP_OK)
    ESP_LOGE("SCORE", "Failed to set last_total: err=0x%x", err);
  else
    ESP_LOGI("SCORE", "Last total saved: %d", total);
  ...
  if (err != ESP_OK || score > best_score) {
    ...
    if (err != ESP_OK)
      ESP_LOGE("SCORE", "Failed to set best_score: err=0x%x", err);
    else
      ESP_LOGI("SCORE", "Best score saved: %d", score);

    err = nvs_set_i32(nvs, key_best_total, total);
    if (err != ESP_OK)
      ESP_LOGE("SCORE", "Failed to set best_total: err=0x%x", err);
    else
      ESP_LOGI("SCORE", "Best total saved: %d", total);
  } else {
    ESP_LOGI("SCORE",
             "Current score %d is not higher than best score %ld, not updating "
             "best.",
             score, best_score);
  }
...
char *get_score_string(int game_time, int game_mode, bool best) {
  ...
  snprintf(key_score, sizeof(key_score), "s%d%d_%s", game_time, game_mode,
           best ? "bs" : "ls");
  snprintf(key_total, sizeof(key_total), "s%d%d_%s", game_time, game_mode,
           best ? "bt" : "lt");
...
static void timer_update_cb(lv_timer_t *timer) {
  if (is_timer == false) {
    ...
    char result[64];
    snprintf(result, sizeof(result), "score %d/%d", correct_counter,
             correct_counter + incorrect_counter);
    save_score(timer_ind, launch_ind, correct_counter,
               correct_counter + incorrect_counter);
