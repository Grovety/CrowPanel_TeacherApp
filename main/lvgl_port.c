...
static void *get_next_frame_buffer(esp_lcd_panel_handle_t panel_handle) {
  ...
  if (next_fb == NULL) {
    ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(
        panel_handle, 2, &fb[0], &fb[1])); // Get the frame buffers
...
IRAM_ATTR static void rotate_copy_pixel(const uint16_t *from, uint16_t *to,
  ...
  switch (rotation) {
  case 90:
    to_index_const = (w - x_start - 1) *
                     h; // Calculate constant index for 90-degree rotation
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {
      from_index = from_y * w + x_start; // Calculate index in the source buffer
      to_index =
          to_index_const + from_y; // Calculate index in the destination buffer
  ...
  case 180:
    to_index_const =
        h * w - x_start - 1; // Calculate constant index for 180-degree rotation
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {
      from_index = from_y * w + x_start; // Calculate index in the source buffer
      to_index = to_index_const -
                 from_y * w; // Calculate index in the destination buffer
      for (int from_x = x_start; from_x < x_end + 1; from_x++) {
        *(to + to_index) = *(from + from_index); // Copy pixel
        from_index += 1; // Move to the next pixel in the source
    ...
    break;
  case 270:
    to_index_const = (x_start + 1) * h -
                     1; // Calculate constant index for 270-degree rotation
    for (int from_y = y_start; from_y < y_end + 1; from_y++) {
      from_index = from_y * w + x_start; // Calculate index in the source buffer
      to_index =
          to_index_const - from_y; // Calculate index in the destination buffer
...
static void flush_dirty_save(lv_port_dirty_area_t *dirty_area) {
  lv_disp_t *disp =
      _lv_refr_get_disp_refreshing(); // Get the currently refreshing display
  dirty_area->inv_p = disp->inv_p;    // Save the number of invalid areas
  for (int i = 0; i < disp->inv_p; i++) {
    dirty_area->inv_area_joined[i] =
        disp->inv_area_joined[i];                  // Save joined areas status
 ...
 *
 * @note This function is used to avoid tearing effect, and only works with LVGL
 * direct mode.
 *
 */
static lv_port_flush_probe_t flush_copy_probe(lv_disp_drv_t *drv) {
  static lv_port_flush_status_t prev_status =
      FLUSH_STATUS_PART;              // Previous flush status
...
static void flush_dirty_copy(void *dst, void *src,
  ...
  lv_coord_t x_start, x_end, y_start,
      y_end; // Coordinates for the area to be copied
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  lv_port_flush_probe_t probe_result =
      FLUSH_PROBE_PART_COPY;               // Default probe result
  ...
  if (lv_disp_flush_is_last(drv)) {
    ...
    if (drv->full_refresh) {
      ...
      if (probe_result == FLUSH_PROBE_FULL_COPY) {
        ...
        disp->rendering_in_progress =
            false;                // Mark rendering as not in progress
        lv_disp_flush_ready(drv); // Mark flush as ready

        /* Force to refresh the whole screen, will invoke `flush_callback`
         * recursively */
...
static void *lvgl_port_flush_next_buf =
    NULL; // Pointer for the flush next buffer
#endif
...
void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
...
#if EXAMPLE_LVGL_PORT_ROTATION_DEGREE != 0
  void *next_fb =
      get_next_frame_buffer(panel_handle); // Get the next frame buffer

  /* Rotate and copy dirty area from the current LVGL's buffer to the next RGB
   * frame buffer */
  rotate_copy_pixel((uint16_t *)color_map, next_fb, offsetx1, offsety1,
                    offsetx2, offsety2, LV_HOR_RES, LV_VER_RES,
                    EXAMPLE_LVGL_PORT_ROTATION_DEGREE);
  ...
  drv->draw_buf->buf2 =
      lvgl_port_flush_next_buf; // Set buffer 2 to the next flush buffer
  lvgl_port_flush_next_buf =
      color_map; // Update the flush next buffer to color_map
...
#elif (LVGL_PORT_LCD_RGB_BUFFER_NUMS == 3) &&                                  \
    (EXAMPLE_LVGL_PORT_ROTATION_DEGREE != 0)
  // Using three frame buffers, one for LVGL rendering and two for RGB driver
  // (one used for rotation)
  void *fbs[3];
  ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 3, &fbs[0],
                                                     &fbs[1], &fbs[2]));
  buf1 = fbs[2]; // Set buf1 to the third frame buffer
#else
  ESP_ERROR_CHECK(esp_lcd_rgb_panel_get_frame_buffer(
      panel_handle, 2, &buf1, &buf2)); // Get two frame buffers
#endif
#else
  // Normally, for RGB LCD, just one buffer is used for LVGL rendering
  buffer_size =
      LVGL_PORT_H_RES * LVGL_PORT_BUFFER_HEIGHT; // Calculate buffer size
...
static esp_err_t tick_init(void) {
  ...
  return esp_timer_start_periodic(lvgl_tick_timer, LVGL_PORT_TICK_PERIOD_MS *
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle,
  ...
  lvgl_mux =
      xSemaphoreCreateRecursiveMutex(); // Create a recursive mutex for LVGL
  assert(lvgl_mux);                     // Ensure mutex creation was successful

  ESP_LOGI(TAG, "Create LVGL task"); // Log task creation
  BaseType_t core_id =
      (LVGL_PORT_TASK_CORE < 0)
          ? tskNO_AFFINITY
          : LVGL_PORT_TASK_CORE; // Determine core ID for the task
  BaseType_t ret =
      xTaskCreatePinnedToCore(lvgl_port_task, "lvgl", LVGL_PORT_TASK_STACK_SIZE,
...
bool lvgl_port_lock(int timeout_ms) {
  assert(
      lvgl_mux &&
      "lvgl_port_init must be called first"); // Ensure the mutex is initialized
...
bool lvgl_port_notify_rgb_vsync(void) {
  ...
  if (lvgl_port_rgb_next_buf != lvgl_port_rgb_last_buf) {
    lvgl_port_flush_next_buf =
        lvgl_port_rgb_last_buf; // Set next buffer for flushing
